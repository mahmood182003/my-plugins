...
public class FastDateFormatFactoryImpl implements FastDateFormatFactory {
	...
	public DateFormat getDate(int style, Locale locale, TimeZone timeZone) {

		String key = getKey(style, locale, timeZone);

		DateFormat format = (DateFormat) _dateFormats.get(key);

		if (format == null) {
			format = I18nFastDateFormat.getDateInstance(style, timeZone, locale);
	...
	public Format getDateTime(int dateStyle, int timeStyle, Locale locale, TimeZone timeZone) {
		...
		if (format == null) {
			format = I18nFastDateFormat.getDateTimeInstance(dateStyle, timeStyle, timeZone, locale);
	...
	public Format getDateTime(Locale locale, TimeZone timeZone) {
		...
		return getDateTime(FastDateFormatConstants.SHORT, FastDateFormatConstants.SHORT, locale, timeZone);
	}
	...
	public Format getSimpleDateFormat(String pattern, Locale locale, TimeZone timeZone) {
		...
		if (format == null) {
			format = I18nFastDateFormat.getInstance(pattern, timeZone, locale);
	...
	public Format getTime(int style, Locale locale, TimeZone timeZone) {
		...
		if (format == null) {
			format = I18nFastDateFormat.getTimeInstance(style, timeZone, locale);
