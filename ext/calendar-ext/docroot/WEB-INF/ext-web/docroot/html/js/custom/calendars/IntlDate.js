...
Calendar.supportedTypes = new Array(4, "Persian", 6, "Hijri", 7, "Buddhist", 8, "Hebrew", 16, "SakaEra", 22,
		"EastAsian", 24, "Japanese", 26, "Julian", 28, "Korean", 30, "Taiwan", 32, "UmAlQura"
...
// Converts digits in a string or number to any of the Unicode digit
// representations.
...
function StringChangeDigitShapes(shape, num) {
	...
	sNum = new String();
	zeros = new Array(0x0030, 0x0660, 0x06F0, 0x0966, 0x09E6, 0x0A66, 0x0AE6, 0x0B66, 0x0BE6, 0x0C66, 0x0CE6, 0x0D66,
			0x0E50, 0x0ED0, 0x0F20, 0x1040, 0x17E0, 0x1810, 0x1946, 0x19D0, 0xFF10, 0x104A0);
...
// returns: An <IntlDate> object. Returns this object if caltype equals 1 or is
// not supported.
...
function IntlDateBase(year, month, day, hr, min, sec, caltype) {
	if (this.constructor.name.startsWith("IntlDate")) { // .ctor called with a
		// new keyword
		var args = "";
		...
		if (caltype === undefined) {
			// throw new Error("'caltype' not specified in the constructor
			// call.");
			caltype = args.substr(args.lastIndexOf(',') + 1, args.length);
		}
		...
		if (calName) {
			try {
				...
				return eval("new " + calName + "Date(" + args + ");");
			} catch (ex) {
		...
		if (arguments.length === 1) { // first param is a Date object, an
			// IntlDate object, or msTicks
			var param1 = year;
...
// Virtual methods can be overridden or use the default implementation in the
// base class.
...
function IntlDateBaseToDateString() {
	var str = "Year: " + this.getFullYear() + ", Mon: " + (this.getMonth() + 1) + ", Day: " + this.getDate();
	return str;
}
...
function IntlDateBaseToTimeString() {
	var str = "hour: " + this.getHours() + ", min: " + this.getMinutes() + ", sec: " + this.getSeconds();
	return str;
}
...
function IntlDateBaseToLocaleDateString() {
	return this.toDateString().changeDigitShapes(Calendar.digitShape);
}
...
function IntlDateBaseToLocaleTimeString() {
	return this.toTimeString().changeDigitShapes(Calendar.digitShape);
}
...
function IntlDateBaseToLocaleString() {
	return this.toString().changeDigitShapes(Calendar.digitShape);
}
...
function IntlDateBaseToUTCString() {
	this.ensureDecode();
	var str = "Year: " + this.getUTCFullYear() + ", Mon: " + (this.getUTCMonth() + 1) + ", Day: " + this.getUTCDate()
			+ "\rhour: " + this.getUTCHours() + ", min: " + this.getUTCMinutes() + ", sec: " + this.getUTCSeconds();
	return str;
}
...
function IntlDateBaseIsEmpty() {
	return (!this.yr) && (!this.mon) && (!this.day) && (!this.hr) && (!this.min) && (!this.sec);
}
...
function IntlDateBaseCopyFrom(date) {
	this.reset(date.yr, date.mon, date.day, date.hr, date.min, date.sec);
...
function IntlDateBaseSetHours(hr, min, sec, msec) {
	...
	if (hr == null)
		hr = msCurHours / Calendar.msHour;
	if (min == null)
		min = (msCurHours % Calendar.msHour) / Calendar.msMinute;
	if (sec == null)
		sec = (msCurHours % Calendar.msMinute) / Calendar.msSecond;
	if (msec == null)
		msec = msCurHours % Calendar.msSecond;
	hr = fix(hr);
	min = fix(min);
	sec = fix(sec);
	msec = fix(msec);
	...
	delta = time.getTime() - msCurHours + 1;
	if (delta != 0)
