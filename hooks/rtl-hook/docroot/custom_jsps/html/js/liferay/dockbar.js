AUI
		.add(
				...
				function(A) {
					...
					var Dockbar = {
						init : function() {
							...
							if (dockBar) {
								...
								instance._namespace = dockBar
										.attr('data-namespace');

								Liferay.once('initDockbar', instance._init,
										instance);

								var eventHandle = dockBar.on([ 'focus',
										'mousemove', 'touchstart' ], function(
										event) {
						...
						addItem : function(options) {
							...
							if (options.url) {
								options.text = '<a href="' + options.url + '">'
										+ options.text + '</a>';
							}

							var item = A.Node.create('<li class="'
									+ (options.className || '') + '">'
									+ options.text + '</li>');
						...
						addMessage : function(message, messageId) {
							...
							message = instance._createMessage(message,
									messageId);
						...
						_addMenu : function(options) {
							...
							if (name && A.one(options.trigger)) {
								...
								if (themeDisplay.getLanguageId() == 'fa_IR') {
									A.mix(options, {
										align : {
											points : [ 'tr', 'br' ]
										},
								...
								if (boundingBox && !(CONTENT_BOX in options)) {
									options.contentBox = boundingBox
											+ '> .aui-menu-content';
								}
								...
								contentBox.plug(A.Plugin.NodeFocusManager, {
									circular : false,
									descendants : 'a',
								...
								contentBox.delegate('mouseenter', function(
										event) {
									focusManager.focus(event.currentTarget
											.one('a'));
								}, '.aui-menu-item');

								contentBox.delegate('mouseleave', function(
										event) {
									focusManager.blur(event.currentTarget
											.one('a'));
								}, '.aui-menu-item');
								...
								menu.on('visibleChange', function(event) {
									...
									trigger.toggleClass('menu-button-active',
											visible);
								});
						...
						_createCustomizationMask : function(column) {
							...
							var customizable = !!column
									.one('.portlet-column-content.customizable');
							...
							if (customizable) {
								overlayMask.get(BOUNDING_BOX).addClass(
										'customizable');
							}

							var columnControls = instance._controls.clone();

							var input = columnControls
									.one('.layout-customizable-checkbox');
							var label = columnControls.one('label');

							var oldName = input.attr('name');
							var newName = oldName.replace('[COLUMN_ID]',
									columnId);
							...
							overlayMask.get(BOUNDING_BOX).prepend(
									columnControls);
						...
						_createMessage : function(message, messageId) {
							...
							return '<div class="dockbar-message ' + cssClass
					...
					Liferay.provide(Dockbar, 'addMenu', function(options) {
						var instance = this;

						instance._addMenu(options);
					}, [ 'aui-overlay-context', 'node-focusmanager' ]);

					Liferay.provide(Dockbar, 'addUnderlay', function(options) {
						var instance = this;
					...
					Liferay
							.provide(
									...
									function() {
										...
										dockBar
												...
												.on(
														...
														function(event) {
															...
															BODY
																	.toggleClass('lfr-dockbar-pinned');

															var pinned = BODY
																	.hasClass('lfr-dockbar-pinned');
										...
										messages
												.on(
														...
														function(event) {
															if (event.newVal) {
																BODY
																		.addClass('showing-messages');

																MenuManager
																		.hideAll();
															} else {
																BODY
																		.removeClass('showing-messages');
															}
														});

										messages.closeTool.on(EVENT_CLICK,
										...
										if (addContent) {
											addContent.on('show', function() {
												Liferay.fire('initLayout');
												Liferay.fire('initNavigation');
											});

											var addContentNode = addContent
													.get(CONTENT_BOX);

											instance._addContentNode = addContentNode;

											var commonItems = addContentNode
													.one('.common-items');

											if (commonItems) {
												commonItems
														.removeClass('aui-menu-item');
											}
											...
											addContentNode.focusManager.set(
													'descendants',
													'a:not(.lfr-portlet-used)');

											Liferay
													.on(
															'closePortlet',
															function(event) {
																var item = addContentNode
																		.one('.app-shortcut[data-portlet-id='
																				+ event.portletId
																				+ ']');
										...
										var userOptionsContainer = A.one('#'
												+ namespace
											...
											addApplicationLink
													.on(
															...
															function(event) {
																...
																if (!addApplication) {
																	var setAddApplicationUI = function(
																			visible) {
																		BODY
																				.toggleClass(
																						'lfr-has-sidebar',
																	...
																	addApplication = instance
																			._addUnderlay({
																				...
																				io : {
																					...
																					data : {
																						doAsUserId : themeDisplay
																								.getDoAsUserIdEncoded(),
																						p_l_id : themeDisplay
																								.getPlid(),
																						p_p_id : 87,
																						p_p_state : 'exclusive',
																						runtimePortletIds : Liferay.Portlet.runtimePortletIds
																								.join()
																					},
																					uri : themeDisplay
																							.getPathMain()
																	...
																	addApplication
																			.after(
																					...
																					function(
																						...
																						if (event.newVal) {
																							Util
																									.focusFormField('#layout_configuration_content');
																						}
											...
											manageContent
													...
													.delegate(
															...
															function(event) {
																...
																var fullDialog = event.currentTarget
																		.ancestor(
																				'li')
																		.hasClass(
																				'full-dialog');
										...
										var manageCustomizationLink = A.one('#'
												+ namespace
												+ 'manageCustomization');
										...
										if (manageCustomizationLink) {
											if (!manageCustomizationLink
													.hasClass('disabled')) {
												instance._controls = dockBar
														.one('.layout-customizable-controls');
																...
																function(event) {
																	...
																	columns
																			.each(function(
																					...
																					index,
																					collection) {
																				var overlayMask = item
																						.getData('customizationControls');

																				if (!overlayMask) {
																					overlayMask = instance
																							._createCustomizationMask(item);
																				}
										...
										var myAccount = A.one('#' + namespace
												+ 'userAvatar .user-links');
											...
											myAccount
													.delegate(
															...
															function(event) {
																...
																var controlPanelCategory = Lang
																		.trim(currentTarget
																				.attr('data-controlPanelCategory'));

																var uri = currentTarget
																		.attr('href');
																var title = currentTarget
																		.attr('title');
										...
										dockBar._menuButtons = dockBar
												.all('ul.aui-toolbar > li > a, .user-links a, .sign-out a');

										dockBar.delegate('keydown', function(
												event) {
											instance._updateMenu(event,
													event.currentTarget);
										}, '.aui-toolbar a');
					...
					Liferay.provide(Dockbar, '_loadAddApplications', function(
							event, id, obj) {
						var contentBox = Dockbar.addApplication
								.get(CONTENT_BOX);
					...
					Liferay
							.provide(
									...
									function(event) {
										...
										var overlayMask = checkbox
												.getData('customizationControls');

										var boundingBox = overlayMask
												.get(BOUNDING_BOX);
										...
										data[checkboxName] = checkbox
												.attr('checked');

										A.io.request(themeDisplay.getPathMain()
												+ '/portal/update_layout', {
											data : data
										});
									}, [ 'aui-io-request' ]);
