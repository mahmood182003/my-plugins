...
A.mix(A.DataType.DateMath, {
	...
	*/
	add : function(date, field, amount) {
		var d = new PersianDate(date.getTime());
		switch (field) {
			case this.MONTH:
	...
	*/
	getWeekNumber : function(date, firstDayOfWeek, janDate) {
		...
		endOfWeek = new PersianDate(startOfWeek.getTime() + 6*this.ONE_DAY_MS);
	...
	isToday: function(date) {
		return !this.isDayOverlap(date, new PersianDate());
	},
	...
	*/
	 ...
	 * are set to 0. The method allows Date instances to be created with the a year less than 100. "new PersianDate(year, month, date)" implementations
	 * set the year to 19xx if a year (xx) which is less than 100 is provided.
	 * <p>
	 * <em>NOTE:</em>Validation on argument values is not performed. It is the caller's responsibility to ensure
	 * arguments are valid as per the ECMAScript-262 Date object specification for the new PersianDate(year, month[, date]) constructor.
	 * </p>
	 * @method getDate
	 * @param {Number} y Year.
	...
	toUsTimeString: function(date, padHours, omitMinutes, hideAmPm) {
		date = isDate(date) ? date : new PersianDate(0, 0, 0, date);
